    ...
    boolean extractseqs(File[] infiles,String extending){
        ...
        for(int i=0;i<nofiles;i++){
            try{
                ...
                //now check each name you write to file to see if it matches idname. if so, put the part
                    ...
                    if(seqnames.containsKey(myfiles[i].seqnames[j])){
                        ...
                        if(currfasta.name.equals("")==false){
                            if(currfasta.name.indexOf(idname)>-1){
                                ...
                                for(int k=java.lang.reflect.Array.getLength(tmparr);--k>=0;){
                                    if(tmparr[k].indexOf(idname)>-1){
                                        //System.out.println("printing sequence for "+idname);
                                        //System.out.println(">"+tmparr[k]+" "+currfasta.name.substring(1));
                                        //System.out.println(currfasta.seq.toUpperCase());
