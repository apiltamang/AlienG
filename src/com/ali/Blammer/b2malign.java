...
public class b2malign {
    ...
    public String[] malign(BufferedReader inread,float coverage,double blastmax,double scval,float minsim){
        ...
        try{
            while (((str=inread.readLine())!=null)&&(stopread==false)){
                ...
                if((index1=str.indexOf("Expect ="))>-1){
                    //now I need to differentiate between blast and psiblast which differ in their outputs
                    if((index2=str.indexOf("Method:"))>-1){//in this case i'm using psiblast
                        String tmpeval=str.substring(index1+8,str.indexOf(",",index1)).trim();
                        if(tmpeval.charAt(0)=='e'){
                            tmpeval="1"+tmpeval;
                        }
                        try{
                            evalue=Double.parseDouble(tmpeval);
                        }catch (NumberFormatException e){
        ...
        if(queryname.length()==0){//can happen, sometimes the blast results are truncated or parts are missing
            System.err.println("No queryname found");
            return new String[0];
        }
        return outarr;
    }// end malign
    ...
    String extractname(String instr){
        ...
        for(int i=java.lang.reflect.Array.getLength(singlenames);--i>0;){
            ...
            if(singlenames[i].matches(".*<a\\s*href=\".*?\"\\s*>.*")){
                ...
                if(singlenames[i].startsWith("</a>")){
                    ...
                    System.out.println("\t\tname '"+singlenames[i]+"' does not start with </a>");
                }
            }
        }// end for i
        for(int i=tmpvec.size();--i>=0;){
            ...
            if((((String)tmpvec.elementAt(i)).indexOf(queryname))>-1){//if this name contains the query
                //System.out.println("'"+(String)tmpvec.elementAt(i)+"' (checkname) contains '"+queryname+"'");
                return ((String) tmpvec.elementAt(i));
            }
            if(queryname.indexOf(((String)tmpvec.elementAt(i)))>-1){//if the query contains the name
                //System.out.println("'"+queryname+"' contains checkname '"+(String)tmpvec.elementAt(i)+"'");
                return queryname;
            }
        }
        if(tmpvec.size()<1){
        ...
        //System.out.println("**********outname="+(String)tmpvec.elementAt(tmpvec.size()-1));
        //as I added the names to the vector in reverse order, I want to return the last name added
        return ((String)tmpvec.elementAt(tmpvec.size()-1));
    }// end extractname
                ...
                forloop:
                    for(currpos=startquery+6;currpos<endthis;currpos+=1){
                        ...
                        if((Character.isJavaIdentifierStart(c))||(c=='-')||(c=='*')){
                            tmpstart=currpos;
                            while ((Character.isJavaIdentifierStart(c))||(c=='-')||(c=='*')){
                                if(c=='-'){
                                    querygaps+=1;
                                }
                                currpos+=1;
