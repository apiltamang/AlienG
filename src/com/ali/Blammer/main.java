...
public class main {
    ...
    String singlefile="undefined";//do I want to use only one file (overrides the other file-selection settings)
    ...
    void run(){
        ...
        while (filesstarted<numberoffiles){
            ...
            synchronized(this){
                ...
                }else{//i.e. at least one thread is done
    ...
    String[] getfilenames(String inname){
        //see if I am using a single file or multiple files
        if(singlefile.equalsIgnoreCase("undefined")==false){
            //then I ONLY WANT TO DO THIS ONE FILE
            String[] outarr=new String[1];
            outarr[0]=singlefile;
            if((new File(singlefile)).canRead()==false){
                System.err.println("IOERROR reading from '"+singlefile+"'");
                System.exit(1);
            }
            return outarr;
        }else{
    ...
    String[] extractnamefromdb(String instr){
        ...
        while((((currpos=instr.indexOf("gi|",maxpos))>-1)||((currpos=instr.indexOf("gt|",maxpos))>-1))&&(specdone==false)){
            ...
            if(specdone==false){//then check the alternative species detection method
    ...
    boolean readargs(String[] args){
        ...
        for(int i=0;i<argslength;i++){
            ...
            if((args[i].equalsIgnoreCase("-file"))||(args[i].equalsIgnoreCase("-singlefile"))){
                i+=1;
                if(i>=argslength){
                    System.err.println("Error at -singlefile, missing argument!");
                    return false;
                }
                singlefile=args[i];
                continue;
                    ...
                    try{
                        ...
                        if((new File(outfname).exists()==false)||(myredo)){
                            ...
                            if(doclustal){
                                ...
                                if(namesseqs==null){//i.e. I had an error in the clustal alignment process
                                    System.err.println("ERROR; alignment unreadable; skipping");
                                    status="killed";
                                    synchronized(parent){
                                        if(verbose>2){
                                            System.out.println("notifying parent for "+fname);
                        ...
                        if((filetest.exists())&&(myredo==false)){
                            //the see if this file is complete, else restart
                            boolean fileok=false;
                            try{
                                BufferedReader inread=new BufferedReader(new FileReader(filetest));
                                //check whether the last non-empty line was "//"
                                String inline;
                                ...
                                System.err.println("IOerror reading from "+filetest.getAbsolutePath());
                            }
                            if(fileok){
                                if(verbose>2){
                        ...
                        if(((new File(hmmfname).exists()==false)||(myredo))&&(dohmmbuild)&&(errorless)){
                            ...
                            try{
                                ...
                                try{
                                    PrintWriter tmpout=new PrintWriter(new BufferedWriter(new FileWriter(cpunum+".commandfile")));
                                    tmpout.println("doing:");
                                    for(int i=0;i<java.lang.reflect.Array.getLength(cmdarr);i++){
                                        tmpout.print(cmdarr[i]+"; ");
                                    }
                                    tmpout.close();
                                }catch (IOException ioe){
                                    System.err.println("cannot write to "+cpunum+".commandfile");
                                }
                        ...
                        if(((filetest.exists())&&(myredo))||(filetest.length()==0)){
                            filetest.delete();
                        }
                        if((filetest.exists())&&(myredo==false)&&(filetest.length()>0)){
                            boolean fileok=false;
                            try{
                                BufferedReader inread=new BufferedReader(new FileReader(filetest));
                                //check whether the last non-empty line was "//"
                                String inline;
                                while((inline=inread.readLine())!=null){
                                    if(inline.equalsIgnoreCase("Histogram of all scores:")){
                                        fileok=true;
                                    }
                                }
                                inread.close();
                        ...
                        if(((new File(hmmsfname).exists()==false)||(new File(hmmsfname).length()==0)||(myredo))&&(dohmmsearch)&&(errorless)){
                            ...
                            try{
                                ...
                                while ((((inout.toString()).trim()).length()==0)&&(redid<5)){
                                    ...
                                    BufferedReader pin=new BufferedReader(new InputStreamReader(p.getInputStream()));
                                    BufferedReader perr=new BufferedReader(new InputStreamReader(p.getErrorStream()));
                                    errread=new threadstreamreader(perr,errout);
                                    inread=new threadstreamreader(pin,inout);
                                    errread.start();
                                    inread.start();
                                ...
                                if(redid>=5){
                                    ...
                                    status="aborting, hmmsearch error 5 times";
                                    synchronized(parent){
                                        if(verbose>2){
                                            System.out.println("notifying parent for "+fname);
                                        }
                        ...
                        if(((new File(hlnfname).exists()==false)||(myredo))&&(new File(hmmsfname).exists())&&(errorless)){//dohmmsearch is right here, since this extracts the hmmsearch sequences to a malign file
                            ...
                            try{
                                ...
                                //now filter these sequences for coverage
                                if(coverage>0 && coverage<=1){
                                    //System.out.println("checking coverage for '"+basename+".hln' coverage="+coverage);
                                    int querylength=0;
                                    for(int i=0;i<java.lang.reflect.Array.getLength(retarr)-2;i+=3){
                                        if(retarr[i].startsWith(idstring)){
                                            //System.out.println("found query");
                                            querylength=retarr[i+2].length();
                                            break;
                                        }
                                    }//end for i
                                    int minlength=(int)(querylength*coverage);
                                    //System.out.println("min.necessary length="+minlength);
                                    ArrayList tmpseqs=new ArrayList();
                                    for(int i=2;i<java.lang.reflect.Array.getLength(retarr);i+=3){
                                        //System.out.println("seq="+retarr[i]+" length="+retarr[i].length());
                                        if(retarr[i].length()>=minlength){
                                            //System.out.println("\tadding");
                                            tmpseqs.add(retarr[i-2]);
                                            tmpseqs.add(retarr[i-1]);
                                            tmpseqs.add(retarr[i]);
                                        }else{
                                            //System.out.println("\t***skip***");
                                        }
                                    }//end for i
                                    retarr=(String[])tmpseqs.toArray(new String[0]);
                                }
