            ...
            for(int i=0;i<numberofblocks;i++){
                ...
                try{
                    try{
                        ...
                        tmpout.print(command+" -INFILE="+tmpfile.getAbsolutePath());
                        tmpout.close();
                    ...
                    errread.start();//read from the streams and make sure I don't get overflow crashes
                    inread.start();
                    //output should be in tmpfile.aln&dnd
                    try{// wait for clustal to finish
                        p.waitFor();
                        if(p.exitValue()!=0){
                            System.err.println("Error, non-perfect exit from '"+command+" -INFILE="+tmpfile.getName()+"' in clualign for "+basename);
                        }
                ...
                if(tmpseqarr==null){//i.e. I could NOT read the clustal alignment for any reason
                    return null;
                }
                //System.out.print("-");
