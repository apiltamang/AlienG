...
public class strictextract {
    ...
    static boolean reextract(String[] namesseqs, String extfname, String strictextfname){
        ...
        for(int i=0;i<checknum;i++){
            ...
            underscorepos=checknamestr.indexOf("_");//this gives trouble with affymetrix id's; limit to maxlength of 3
            if((underscorepos>0)&&(underscorepos<4)&&((pipepos<0)||(pipepos>underscorepos))){//if I have a _ before a pipe or alone
        ...
        if(extfile.exists()==false){//if the extractfile isn't there
            ...
            for(int i=0;i<checknum;i++){
                if(((Integer)checknames.get(namearr[i])).intValue()<1){
                    System.err.println("-----name not found:'"+namearr[i]+"' (from '"+namesseqs[i*3]+"') in "+strictextfname);
                }
            }//end for i
        }// end else
        return true;
    ...
    static boolean checkname(String inline, java.util.HashMap checknames){
        ...
        for(int i=0;i<java.lang.reflect.Array.getLength(tmparr);i++){
            ...
            if(testline.indexOf(" ")>-1){//if I have a spcae in the name, keep all up to the space
