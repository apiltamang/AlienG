 ...
 * @version
 */
public class methodsnograph {
    
    /** Creates new methods */
    public methodsnograph() {
    }
    
    treenode computetree(double[][][] indistances,String[] innames,int verbose){
        ...
        for(int i=0;i<replicates+1;i++){
            ...
            while(java.lang.reflect.Array.getLength(currmtx)>3){
                ...
                nodes=makenextnode(nodes,group,currmtx,diverg,cnt,verbose);
    ...
    treenode[] makenextnode(treenode[] innodes,int[] ingroup,double[][] inmtx,double[] diverg,int cnt, int verbose){
        ...
        for(int i=0;i<species;i++){
            for(int j=i+1;j<species;j++){
                ...
                if(poscount<1){
                    ...
                    System.out.println("poscount=0; no overlap");
                    outarr[i][j]=java.lang.Double.MAX_VALUE;
                }else if((corrmet.equalsIgnoreCase("poisson"))||(corrmet.equalsIgnoreCase("p"))){
                    outarr[i][j]=distance.poisson((double)diff/(double)(poscount));
                }else if((corrmet.equalsIgnoreCase("kimura"))||(corrmet.equalsIgnoreCase("k"))){
                    outarr[i][j]=distance.kimura((double)diff/(double)(poscount));
                }else if((corrmet.equalsIgnoreCase("tajimanei"))||(corrmet.equalsIgnoreCase("tn"))){
                    outarr[i][j]=distance.tajimanei((double)diff/(double)(poscount));
                }else if((corrmet.equalsIgnoreCase("none"))||(corrmet.equalsIgnoreCase("n"))){
                    outarr[i][j]=((double)diff/(double)(poscount));
                }else{
                ...
                if((java.lang.Double.isNaN(outarr[i][j]))||(java.lang.Double.isInfinite(outarr[i][j]))){
                    parent.skipthis=true;
                    System.out.println("dissim="+((double)diff/(double)(poscount)));
                    outarr[i][j]=java.lang.Double.MAX_VALUE;
                }
    ...
    double[][] getdistances(aaseq[] inarr,String corrmet,treemakethreadnograph parent){
        ...
        for(int i=0;i<species;i++){
            for(int j=i+1;j<species;j++){
                ...
                if(poscount<1){
                    parent.skipthis=true;
                    System.err.println("No overlap between sequences("+i+","+j+") in "+parent.dofile.getName());
                    outarr[i][j]=java.lang.Double.MAX_VALUE;
                }else if((corrmet.equalsIgnoreCase("poisson"))||(corrmet.equalsIgnoreCase("p"))){
                    outarr[i][j]=distance.poisson((double)diff/(double)(poscount));
                }else if((corrmet.equalsIgnoreCase("kimura"))||(corrmet.equalsIgnoreCase("k"))){
    ...
    treenode lastparent=new treenode();
    float lastlength;
    float lastboot;
    float lastlastboot;
    ...
    public treenode roottree(treenode innode,String rootname){
        ...
        if(rootname.length()<1){
            System.err.println("no rootname specified: rootname='"+rootname+"'");
            return innode;
        }
    ...
    private treenode rootnode(treenode innode,String rootname){//root current tree with node with name rootname
        ...
        foundroot=false;
        if((innode.name.indexOf("|"+rootname+"|")!=-1)||(innode.name.indexOf(rootname+" ")>-1)||innode.name.equals(rootname)){
            System.out.println("rerooting at nodename:"+innode.name+" for rootname'"+rootname+"'");
        ...
        if(foundroot==false){// but check nevertheless
            for(int i=0;i<innode.kids();i++){
                ...
                if(foundroot==true){
                    rootkid=i;
                    break;//i=java.lang.Integer.MAX_VALUE-1;// terminate the for loop
                }
            }// end for
            if(foundroot==false){
                return innode;
        ...
        System.out.println("error in rerooting node "+innode.name+" foundroot="+foundroot);
        return innode;
    }// end rootnode
