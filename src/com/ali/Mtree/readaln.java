 ...
 * readaln.java
 *
 * Created on September 29, 2002, 10:44 AM
 */
import java.io.*;
import java.util.*;
/**
...
public class readaln {
            ...
            while((inline=inread.readLine())!=null){
                ...
                if(inline.length()>0){//if there is sth. on this line
                    if(inline.startsWith(">")){//if this is fasta format
                        ...
                        //System.out.println("do readclustal");
                        inread.close();
            ...
            while((inline=inread.readLine())!=null){
                ...
                if(inline.startsWith(">")){//if this is a fasta name
                    if(seqbuff.length()>0){//if I have read a seq
                        myaaseq.seqseq=seqbuff.toString().toUpperCase().toCharArray();
                        seqbuff.setLength(0);
                        tmpvec.addElement(myaaseq);
                    }
                    myaaseq=new aaseq();
                    myaaseq.seqname=inline.substring(1);//skip the >
                }else{
    ...
    public static aaseq[] clustalread(File infile){
        ...
        try{
            ...
            while((inline=inread.readLine())!=null){
                ...
                if(inline.length()==0){//if the line is empty
                    ...
                    //System.out.println("clustal line; continue");
                    continue;
                }else{
                    ...
                    if(inline.matches(".*[a-zA-Z-]+.*")==false){//if it has no a-z or gap "-"
                        //System.out.println("skip this info line");
                        continue;
                    }
        ...
        //System.out.println("hashsize="+keysnum);
        aaseq[] retarr=new aaseq[keysnum];
        String[] keys=(String[])(myhash.keySet().toArray(new String[0]));
        for(int i=0;i<keysnum;i++){
            //System.out.println("key"+i+" "+keys[i]);
            retarr[i]=new aaseq();
            retarr[i].seqname=keys[i];
            retarr[i].seqseq=((StringBuffer)myhash.get(keys[i])).toString().toUpperCase().toCharArray();
        }
        return retarr;
    }// end clustalread
    ...
    throws IOException{
        ...
        while (((aaseqname=(alignread.readLine()))!=null)){
            if (((aaseqname.length())!=0)){
                ...
                currentread.seqseq=new String(seqseq).toUpperCase().toCharArray();
        ...
        try{
            ...
            while((readon)&&((inline=inread.readLine())!=null)){//while I can read and have not read the numbers
                ...
                if(inline.length()>0){
                    ...
                    readon=false;
                    //System.out.println("READNUM IN LINE "+inline);
                }
            }
            try{
            ...
            int seqspecs=-1;
            int intspecs=-1;
            //System.out.println("done init "+specnum+" species");
            while(((inline=inread.readLine())!=null)&&(physeq||phyint)){//while I am reading from the file and either seq or int are still ok
                ...
                //try sequential read
                //System.out.println("IN LINE "+inline);
                if(physeq){
                    ...
                    if(seqspecs>=specnum){
                        ...
                        if(seqspecs>-1){
                            int currseqpos=java.lang.reflect.Array.getLength(((aaseq)physeqvec.elementAt(seqspecs)).seqseq);
                            //System.out.println(currseqpos+"=current seq length");
                            if(currseqpos<seqlength){//if I have not read the full sequence yet
                                //System.out.println("-----adding "+inline);
                                ((aaseq)physeqvec.elementAt(seqspecs)).seqseq=(new String(((aaseq)physeqvec.elementAt(seqspecs)).seqseq)+inline).toUpperCase().toCharArray();
                            }
                            if(currseqpos>seqlength){
                                System.err.println("sequence "+((aaseq)physeqvec.elementAt(seqspecs)).seqseq+" is longer than "+seqlength);
                            ...
                            if(currseqpos==seqlength){
                                ...
                                seqspecs++;
                                ((aaseq)physeqvec.elementAt(seqspecs)).seqname=inline.substring(0,10).trim();
                                ((aaseq)physeqvec.elementAt(seqspecs)).seqseq=(inline.substring(10).trim()).replaceAll(" ","").toCharArray();
                            }
                ...
                if(phyint){
                    ...
                    if(intspecs<(specnum-1)){
                        ...
                        intspecs++;
                        ((aaseq) phyintvec.elementAt(intspecs)).seqname=inline.substring(0,10).trim();
                        ((aaseq) phyintvec.elementAt(intspecs)).seqseq=(inline.substring(10).trim()).replaceAll(" ","").toCharArray();
                    }else{
                        ...
                        ((aaseq) phyintvec.elementAt(currspec)).seqseq=(new String(((aaseq) phyintvec.elementAt(currspec)).seqseq)+(inline.trim()).replaceAll(" ","")).toUpperCase().toCharArray();
                        if(java.lang.reflect.Array.getLength(((aaseq) phyintvec.elementAt(currspec)).seqseq)>seqlength){
                            System.err.println("sequence "+((aaseq) phyintvec.elementAt(currspec)).seqname+" is longer than "+seqlength);
                            System.err.println(new String(((aaseq) phyintvec.elementAt(currspec)).seqseq));
        ...
        try{
            ...
            while (((inline=inread.readLine())!=null)&&(doneall==false)){
                ...
                if(inline.startsWith("#")){
                    ...
                    curr=new aaseq();
                    curr.seqname=inline.substring(0,29).trim();
                    curr.seqseq=inline.substring(29).trim().toCharArray();
